...
/* Sessions shorter than this are almost always an open-then-immediately-close
   tap, not real use — recording them would flood Daybook with noise. Below
   this floor the session is dropped entirely (not journaled, not kept in the
   local ledger). */
export const MIN_SESSION_SECONDS = 180;
...
export function createSessionTracker({
  ...
  const publish = (finished) => {
    if (Math.max(0, Number(finished?.activeSeconds) || 0) < MIN_SESSION_SECONDS) return [];
